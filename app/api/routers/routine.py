...
def get_routine(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return crud.get_routine(db, user.id)


@router.patch("", response_model=RoutineOut)
def patch_routine(payload: RoutinePatch, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return crud.patch_routine(db, user.id, payload)
